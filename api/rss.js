// Vercel Edge Function - RSS for Shanxi Daily e-paper
...
export default async function handler(req) {
  ...
  try {
    ...
    const edRe = /href="(\/shtml\/sxrb\/\d{8}\/v\d{2}\.shtml)"/gi;
    const editions = [...new Set([...html.matchAll(edRe)].map(m => m[1]))];
    if (editions.length > 0) {
      const results = await Promise.allSettled(editions.map(e => fetchHTML(EPAPER + e).catch(() => '')));
      for (const r of results) {
        if (r.status === 'fulfilled' && r.value) articles = articles.concat(parseArticles(r.value, today));
      }
    }
    const seen = new Set();
